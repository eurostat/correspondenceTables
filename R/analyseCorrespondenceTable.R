...
# Take the name of the CSV filename of the user in the way to create csv file 
if (!is.null(input_file_path)) {
  base_file_name <- tools::file_path_sans_ext(tools::file_path_sans_ext(basename(input_file_path)))
} else {
  # Generate a unique base file name (e.g., based on timestamp)
  base_file_name <- paste0("correspondence_analysis_", format(Sys.time(), "%Y%m%d%H%M%S"))
}

# Écriture des annexes dans des fichiers CSV si les paramètres sont fournis
if (!is.null(CSVcorrespondenceInventory)) {
  write.csv(annex_A_df, paste0(base_file_name, "_correspondenceInventory.csv"), row.names = FALSE)
  
}

if (!is.null(CSVcorrespondenceAnalysis)) {
  write.csv(annex_B_df, paste0(base_file_name, "_correspondenceAnalysis.csv"), row.names = FALSE)
}
